document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function carregarProdutos() {
        ...
        if (!resposta.ok) {
            throw new Error(dados.erro || 'Erro ao carregar produtos')
        }
    ...
    function criarCardProduto(produto, destaque = false) {
        ...
        const descricao = escaparHTML(produto.descricao_curta || 'Produto digital disponível na Monetiza')
        ...
        card.innerHTML = `
            ...
            <div class="product-info" style="display:flex;flex-direction:column;align-items:stretch;text-align:left;gap:10px;">
                ...
                <span style="color:#7c7c8a;font-size:12px;">Por ${produtor}</span>
    ...
    function renderizarMaisVendidos() {
        ...
        if (produtos.length === 0) {
            listaMaisVendidos.innerHTML = `<p style="color:#7c7c8a;">Ainda não existem produtos disponíveis.</p>`
            return
        }
